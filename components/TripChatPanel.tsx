...
function ChatBubble({
  ...
  return (
    <div className="flex items-start gap-2">
      {/* Initials avatar */}
      <div className="flex-shrink-0 w-7 h-7 rounded-full bg-violet-700/50 flex items-center justify-center text-[10px] font-bold text-violet-200">
        {getInitials(message.user_email)}
      </div>

      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-1.5">
          <span className="text-[11px] font-semibold text-violet-300 truncate max-w-[140px]">
            {message.user_email}
          </span>
...
function ChatTeaser({
  ...
  return (
    <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 flex flex-col items-center text-center gap-2">
      <div className="w-9 h-9 rounded-full bg-violet-900/40 flex items-center justify-center">
      ...
      {onUpgrade && (
        <button
          onClick={onUpgrade}
          className="mt-1 px-4 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-500 text-xs font-bold text-white transition-colors"
        >
          {L.upgradeCta}
        </button>
      )}
        ...
        <>
          ...
          <div className="flex items-end gap-2 px-4 py-3 border-t border-white/[0.06]">
            <textarea
              ...
              rows={1}
              disabled={sending}
              className="flex-1 resize-none rounded-lg border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-1 focus:ring-violet-500/50 disabled:opacity-50 min-h-[36px] max-h-[96px]"
            />
            <button
              onClick={() => void handleSend()}
              disabled={!draft.trim() || sending}
              aria-label={L.send}
              className="flex-shrink-0 flex items-center justify-center w-9 h-9 rounded-lg bg-violet-600 hover:bg-violet-500 disabled:opacity-40 disabled:pointer-events-none transition-colors"
            >
